...
for i in `seq 1 ${length[0]}`; do
    ...
    echo "Start transfer MEM to MEM: $transfer"
    ...
    echo "Running at send node: iperf3 -c $NODE2_IP -i 1 -t $tmax"
    #this is a blocking call
    ssh -i $FDT_PRIV_KEY $Node1 "iperf3 -c $NODE2_IP -i 1 -t $tmax >> $LOG_PATH_GENI/$LOG_SEND_iperf"
